...
export default function Navbar() {
  ...
  return (
    <>
      {/* Dark overlay backdrop — covers whole page when menu open */}
      {isOpen && (
        <div
              ...
              {navItems.map((item) => {
                ...
                return (
                  <Link
                    ...
                    to={item.path}
                    onClick={() => setIsOpen(false)}
                    className={`flex items-center justify-between px-3 py-3 rounded font-mono text-sm transition-colors ${
                      isActive
                        ? 'bg-[#1a1a1a] text-accent font-medium border-l-2 border-accent pl-2.5'
